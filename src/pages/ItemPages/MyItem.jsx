...
const MyItem = () => {
	...
	if (currentUser) {
		userCollectionsItemIdArray = [...currentUser.collections].map((collection) => {
	...
	if (currentUser && currentUser._id && itemCollectionIds && userCollectionsItemIdArray) {
		return (
			<>
					...
					<div id='main-content'>
						<div id='main-section' className='justify-center p-4'>
							<div className='p-4 bg-slate-50 rounded-md'>
									...
									<div>
										{comments.map((comment) => {
											return (
												<div key={comment._id}>
													<h6>
														<Link to={`/users/${comment.user.username}`}>{comment.user.username}</Link> says:
													</h6>
													<p>{comment.title}</p>
													<p>{comment.body}</p>
												</div>
								...
								{userCollectionsItemIdArray.every((item) => item.includes(itemId)) ? (
									<div></div>
								) : (
