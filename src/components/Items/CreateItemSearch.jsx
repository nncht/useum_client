...
const CreateItemSearch = () => {
	...
	useEffect(() => {
		if (selectedOption && selectedOption.title === 'No match? Add a new item to the database ✍️') {
	...
	return (
		...
		<div id='search-bar' className='pt-4'>
			<Autocomplete
				...
				filterOptions={(options, state) => {
					const filteredOptions = options.filter(
						(option) => option.title.toLowerCase().indexOf(state.inputValue.toLowerCase()) !== -1
					);
				...
				renderOption={(props, option) => (
					<li {...props}>
						{option.type === 'item' && <span className='text-slate-700 uppercase'> Item &nbsp;&nbsp;&nbsp;</span>}
						{option.title}
					</li>
				)}
			/>
			...
			{itemDoesntExist ? (
				...
				<div className='mt-10'>
					<CreateItemForm target={'items'} idObject={'item'} forCollection={collectionId} />
				</div>
						...
						{itemsFound.length ? (
							itemsFound.map((item) => (
								<Grid item xs={12} sm={6} md={4} lg={3} key={item._id}>
									...
									<Button onClick={() => handleOpen(item)}>Add this item</Button>
									<Dialog
										open={openModal}
										onClose={handleClose}
									...
									>
										...
										>
											...
											<DialogContent>
												<DialogContentText>
													Let the community know your thoughts about this item, e.g. why you're using it, how much you
													like it, etc.
												</DialogContentText>
